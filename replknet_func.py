...
class ReparamLargeKernelConv(nn.Module):
    ...
    def forward(self, inputs):
        if hasattr(self, 'lkb_reparam'):# small_kernel_mergedが`True`の時。
            out = self.lkb_reparam(inputs)
        else:# small_kernel_mergedが`False`の時。学習・FT時は絶対こちら。
...
def create_RepLKNet31L(drop_path_rate=0.3, num_classes=1000, use_checkpoint=True, small_kernel_merged=False, use_sync_bn=True):
    return RepLKNet(large_kernel_sizes=[31,29,27,13], layers=[2,2,18,2], channels=[192,384,768,1536],
                    drop_path_rate=drop_path_rate, small_kernel=5, num_classes=num_classes, use_checkpoint=use_checkpoint,
                    small_kernel_merged=small_kernel_merged, use_sync_bn=use_sync_bn)

def create_RepLKNetXL(drop_path_rate=0.3, num_classes=1000, use_checkpoint=True, small_kernel_merged=False, use_sync_bn=True):
...
def build_model(model_config, fine_tuned_weights=None):
    ...
    if fine_tuned_weights != None:
        f_tuned_weights = torch.load(fine_tuned_weights, map_location='cpu')
        if 'model' in f_tuned_weights:
            f_tuned_weights = f_tuned_weights['model']
        if 'state_dict' in f_tuned_weights:
            f_tuned_weights = f_tuned_weights['state_dict']

        model.load_state_dict(f_tuned_weights)
        print('loaded fine_tuned_weights')
